...
@else
    <textarea name="{{ $name }}" value="{{ $value }}" placeholder="{{ $placeholder }}" cols="{{ $cols }}" rows="{{ $rows }}" {{ $attributes->merge (['class' => 'mt-2 resize-none w-full rounded-md border-2 border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-gray-700 focus:border-blue-600 dark:focus:border-blue-300'])}}></textarea>
@endif
